...
import { CartType } from "../services/types";
import { useCartChange } from "../services/hook";

export const CartItem: React.FC<CartType> = (props) => {
  const { inc, dec, remove } = useCartChange();
  return (
    <div className="flex md:gap-0 gap-6 md:justify-between md:items-center md:flex-row flex-col border-r border-b border-gray-200 py-4 pr-8">
      ...
      <div className="flex lg:gap-20 gap-4 items-center justify-between md:justify-normal">
        ...
        <button className="text-gray-500" onClick={() => remove(props)}>
          <X size={16} />
        </button>
      </div>
    </div>
